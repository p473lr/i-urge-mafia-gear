...
public class common_problems {
	...
	/**
	 * given a set of elements with weight and value, maximize the value if the
	 * sum of weights cannot exceed a given sum each item can be used an
	 * infinite number of times!
	 */
	static int knapsack_problem(int[] weights, int values[], int sum) {
		int[] arr = new int[sum + 1];
		for (int i = 1; i < arr.length; ++i) {
			// if the weight must be EXACTLY the given sum, remove the following
			// line
			arr[i] = arr[i - 1];
	...
	/**
	 * given a set of elements with weight and value, maximize the value if the
	 * sum of weights cannot exceed a given sum each item can only be used once
	 * or zero times!
	 */
	static int int_knapsack_problem(int[] weights, int[] values, int sum) {
		...
		for (int w = 1; w <= weights.length; ++w) {
			for (int s = 1; s <= sum; ++s) {
				...
				if (s - weights[w - 1] >= 0)
					table[w][s] = Math.max(table[w - 1][s - weights[w - 1]]
							+ values[w - 1], table[w][s]);
			}
		}
	 ...
	 * written with arrays instead of strings to be easily modifiable to work
	...
	static char[] longest_common_substring(char a[], char b[]) {
		...
		// If you would like the indices of the lcs in a and b instead, return
		// this:
